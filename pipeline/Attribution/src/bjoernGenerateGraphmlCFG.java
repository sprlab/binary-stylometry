...
public class bjoernGenerateGraphmlCFG {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ScriptException{
        ...
        String datasetName = new File(testFolder).getName();
		...
		List<File> binary_paths = Util.listBinaryFiles(folderToProcess);
		int noFiles = binary_paths.size();
		System.out.println("Num files: " + noFiles);
		System.out.println("Paths:");
		for (File f: binary_paths) {
			System.out.println("\t" + f.getName());
		}
		int modVal = 1;   //might be number of authors
                          //testing whether this is cause of CFG bug
		...
		for(int i=0; i<iterations; i++){
			try
			{
				...
				for(int i1=0; (i1<= modVal-1) && (i1+(i*modVal) < noFiles); i1++){ // changed: used to be strict inequality
					...
					if((cfgFolder.exists() == false) || (FileUtils.sizeOfDirectory(cfgFolder) < 1)){
						...
						bjoernGenerateCFG(binary_paths.get(i1+(i*modVal)).toString(), datasetName);

					}}


				Thread.sleep(15000); //really? make this more reasonable

				for(int i1=0; (i1<= modVal-1) && (i1+(i*modVal) < noFiles); i1++){

					File bin= new File(binary_paths.get(i1+(i*modVal)).toString());
					File cfgFolder= new File(binary_paths.get(i1+(i*modVal)).toString() + "_bjoernDisassembly" + File.separator +bin.getName()+"CFG"+File.separator);
					if((cfgFolder.exists() == false) || (FileUtils.sizeOfDirectory(cfgFolder) < 1)){
						dumpCFG(binary_paths.get(i1+(i*modVal)).toString(), datasetName);
					}
				}
	...
	public static void bjoernGenerateCFG(String filePath, String datasetName) throws IOException, InterruptedException, ScriptException{
		...
		String dbName = filename +"_" + datasetName + "_" + "CFG" ;
	...
	public static void dumpCFG(String filePath, String datasetName) throws IOException, InterruptedException, ScriptException{
		//generates the cfgs in outdir
		File processing = new File(filePath);
		String path = FilenameUtils.getPath(filePath);
		String filename = processing.getName();
		//String dbName = filename +"CFG" ;
		// Modified to avoid dataraces when running multiple datasets in parallel
		String dbName = filename +"_" + datasetName + "_" + "CFG" ;
		String outdir = File.separator + path + filename +"_bjoernDisassembly" + File.separator + filename + "CFG";
		String outdirTMP = File.separator + path + filename +"_bjoernDisassembly" + File.separator +"bjoernCFG"+File.separator;
		File outputTMP = new File(outdirTMP);
		outputTMP.mkdir();
		...
		Process dumpCFG = dumpTime.exec(new String[]{"/bin/bash", "-c",
				...
				//	Test changing path to avoid matlab sniping
				"LD_LIBRARY_PATH='' curl http://localhost:2480/dumpcfg/"+ dbName + " ;"
		});
		...
		Process cpCFG = cpTime.exec(new String[]{"/bin/bash", "-c",
				"cp -r "+ bjoern_radareFolder.trim() +"/dump/cfg/"+dbName +File.separator+  " " + outdir + " ;"
						...
						+"LD_LIBRARY_PATH='' curl --user root:admin -X DELETE http://localhost:2480/database/"+dbName+" ;"//delete db from orientDB
        // Test changing path to avoid matlab sniping
		...
		File cfgDB = new File (cfgDBFolder);
		// commented out to test if it fixes bug - 11/29
		//if(cfgDB.exists()){
