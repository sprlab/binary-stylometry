...
int deregister_tm_clones()
{
  ...
  result = &unk_804A04B - &edata;
  if ( (unsigned int)(&unk_804A04B - &edata) > 6 )
    return 0;
  return result;
}
...
{
  char v3; // bl
  int v5; // [esp+18h] [ebp-1Ch] BYREF
  char v6[4]; // [esp+1Ch] [ebp-18h] BYREF
  ...
  for ( i = 1; i <= v5; ++i )
  {
    ...
    v10 = std::string::length((std::string *)v6);
    v8 = 0;
    for ( j = 1; j < v10; ++j )
    {
      v3 = *(_BYTE *)std::string::operator[](v6, j);
      if ( v3 != *(_BYTE *)std::string::operator[](v6, j - 1) )
        ++v8;
    }
    if ( *(_BYTE *)std::string::operator[](v6, v10 - 1) == 45 )
      ++v8;
    printf("Case #%d: %d\n", i, v8);
    std::string::~string((std::string *)v6);
  }
...
// 8048690: using guessed type int __cdecl std::string::operator[](_DWORD, _DWORD);
// 80486E0: using guessed type int __cdecl std::operator>><char>(_DWORD, _DWORD);
// 8048710: using guessed type int __cdecl std::istream::operator>>(_DWORD, _DWORD);
// 804884D: using guessed type char var_18[4];
...
// nfuncs=42 queued=13 decompiled=13 lumina nreq=0 worse=0 better=0
