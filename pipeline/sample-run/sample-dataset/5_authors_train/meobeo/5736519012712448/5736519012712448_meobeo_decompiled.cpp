...
int __cdecl main(int argc, const char **argv, const char **envp)
{
  ...
  int n; // [esp+24h] [ebp-10h]
  int m; // [esp+28h] [ebp-Ch]
  int v15; // [esp+2Ch] [ebp-8h]

  std::istream::operator>>(&std::cin, &T);
  for ( i = 1; i <= T; ++i )
  {
    v3 = std::operator>><char>(&std::cin, &S);
    std::istream::operator>>(v3, &K);
    for ( j = 0; j < std::string::size((std::string *)&S); ++j )
      a[j] = *(_BYTE *)std::string::operator[](&S, j) != 45;
    v11 = 0;
    v15 = std::string::size((std::string *)&S);
    for ( k = 0; k < v15; ++k )
    {
      if ( !a[k] )
      {
    ...
    if ( v11 == -1 )
    {
      v6 = std::operator<<<std::char_traits<char>>(v5, ": IMPOSSIBLE");
    }
    else
    {
      v7 = std::operator<<<std::char_traits<char>>(v5, ": ");
      v6 = std::ostream::operator<<(v7, v11);
    }
...
// 80486F0: using guessed type int __cdecl std::string::operator[](_DWORD, _DWORD);
// 8048740: using guessed type int __cdecl std::operator<<<std::char_traits<char>>(_DWORD, _DWORD);
// 8048750: using guessed type int __cdecl std::operator>><char>(_DWORD, _DWORD);
...
// 804A1A4: using guessed type int K;
// 804A1C0: using guessed type int a[10001];
...
void __cdecl __static_initialization_and_destruction_0(int __initialize_p, int __priority)
{
  if ( __initialize_p == 1 && __priority == 0xFFFF )
  {
    ...
    __cxa_atexit(std::string::~string, &S, &_dso_handle);
  }
}
