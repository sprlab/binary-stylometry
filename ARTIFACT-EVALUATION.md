...
- Extract the corresponding machine‑code bytes and translated machine code to ASCII.
...
- Classify the feature as code‑based if it lay in .text (or appeared in both code and non‑code sections), otherwise non‑code‑based.
- If a feature occured in non-code based sections, use itss ASCII representation to detect patterns like function calls, symbol usage, etc. 

#### radare2‑based features

- Align each feature’s text against the radare2 disassembly files (nodes.csv file in the intermediate data) to locate the instructions.
...
- Align each feature’s text to its location in the decompiled C++ file. 
